...
def load_data():
    """데이터 로드"""
    try:
        flow_df = pd.read_csv('/mnt/c/Users/rladl/Desktop/bigcontest_2025/2025_shcard_bigcontest/data/유동인구.csv')
        rent_df = pd.read_csv('/mnt/c/Users/rladl/Desktop/bigcontest_2025/2025_shcard_bigcontest/data/임대료.csv')
        integrated_df = pd.read_csv('/mnt/c/Users/rladl/Desktop/bigcontest_2025/2025_shcard_bigcontest/data/통합_제공데이터.csv')
        # 기준일ID를 날짜로 변환
...
def main():
    ...
    with col3:
        ...
        if competitive_fig:
            st.plotly_chart(competitive_fig, width="stretch")
    ...
    with col3:
        ...
        if customer_fig:
            st.plotly_chart(customer_fig, width="stretch")
    ...
    with col2:
        if flow_df is not None:
            ...
            if district_code:
                ...
                with col1:
                    if fig_flow1:
                        st.plotly_chart(fig_flow1, width="stretch",key="sales_trend")
                
                with col2:
                    if fig_flow2:
                        st.plotly_chart(fig_flow2, width="stretch", key="delivery_ratio")
                
                with col3:
                    if fig_flow3:
                        st.plotly_chart(fig_flow3, width="stretch", key="competitive_position")
