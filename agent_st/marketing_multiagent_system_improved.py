...
# GRPC 및 로깅 경고 메시지 무시 (모든 import 전에 설정)
import os
...
class ClusterProfile(BaseModel):
    """클러스터 프로파일"""
    cluster_id: str  # 'Cluster 1' 형태의 문자열
...
class STPOutput(BaseModel):
    ...
    # Targeting
    target_cluster_id: str  # 'Cluster 1' 형태의 문자열
...
class STPDataLoader:
    ...
    def get_pc_interpretation(self, industry: str) -> Dict[str, PCAxisInterpretation]:
        ...
        return {
            'PC1': PCAxisInterpretation(
                axis='PC1',
                interpretation=f"{pc1_features[0]['속성']} vs {pc1_features[1]['속성']}" if len(pc1_features) >= 2 else '데이터 부족',
                top_features=pc1_features
            ),
            'PC2': PCAxisInterpretation(
                axis='PC2',
                interpretation=f"{pc2_features[0]['속성']} vs {pc2_features[1]['속성']}" if len(pc2_features) >= 2 else '데이터 부족',
...
def segmentation_agent_node(state: MarketAnalysisState) -> MarketAnalysisState:
    ...
    for cluster in cluster_data:
        # CSV 컬럼명 매핑
        cluster_id = cluster.get('클러스터 ID', cluster.get('cluster_id', 0))
        pc1_mean = cluster.get('PC1 평균 (X)', cluster.get('pc1_mean', 0))
        pc2_mean = cluster.get('PC2 평균 (Y)', cluster.get('pc2_mean', 0))
        store_count = cluster.get('경쟁 그룹 수', cluster.get('store_count', 0))
...
"""
        ...
        cluster_name = lines[0].replace('1.', '').strip() if len(lines) > 0 else f"그룹 {cluster_id}"
    ...
    print(f"⏱️  [Segmentation Agent] 완료 - 소요시간: {elapsed_time:.2f}초")
...
def targeting_agent_node(state: MarketAnalysisState) -> MarketAnalysisState:
    ...
    current_position = StorePosition(
        ...
        cluster_id=str(store_info['cluster_id']),  # 문자열로 유지
        cluster_name="",  # 나중에 매칭
        competitor_count=0
    )
    ...
    elapsed_time = time.time() - start_time
    print(f"⏱️  [Targeting Agent] 완료 - 소요시간: {elapsed_time:.2f}초")
...
"""
    ...
    elapsed_time = time.time() - start_time
    print(f"⏱️  [Positioning Agent] 완료 - 소요시간: {elapsed_time:.2f}초")
...
## Positioning
- 추천 포지션: PC1={f'{stp.recommended_white_space.pc1_coord:.2f}' if stp.recommended_white_space else 'N/A'}, PC2={f'{stp.recommended_white_space.pc2_coord:.2f}' if stp.recommended_white_space else 'N/A'}
- 이유: {stp.recommended_white_space.reasoning if stp.recommended_white_space else '분석 중'}
...
"""
    ...
    elapsed_time = time.time() - start_time
    print(f"⏱️  [Strategy Agent] 완료 - 소요시간: {elapsed_time:.2f}초")
...
"""
    ...
    elapsed_time = time.time() - start_time
    print(f"⏱️  [Content Agent] 완료 - 소요시간: {elapsed_time:.2f}초")
...
def create_super_graph() -> StateGraph:
    ...
    workflow.add_node("strategy_team", lambda s: {
        "strategy_4p": strategy_team.invoke(s)['strategy_4p'],
        "positioning_concept": strategy_team.invoke(s)['positioning_concept'],
        "execution_plan": strategy_team.invoke(s)['execution_plan']
    })
...
def run_marketing_strategy_system(
    ...
    print(f"⏱️  전체 소요시간: {total_elapsed_time:.2f}초 ({total_elapsed_time/60:.2f}분)")
    print("=" * 80)
    ...
    return {
        ...
        "positioning_concept": final_state['positioning_concept'],
